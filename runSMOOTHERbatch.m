function jobs = runSMOOTHERbatch(gfdfile,cluster,clusterpaths,MagicConstFile,hdf5)
%
% jobs = runSMOOTHERbatch(gfdfile,cluster,clusterpaths,MagicConstFile,hdf5)
%
% run bafim_smoother as batch jobs in a cluster
%
%
% INPUT:
...
%
% NB! the magic constant has no effect on the smoother, it is kept only for similarity with runGUISDAPbatch
%
%
% IV 2022
%
    ...
    while t1new < t2datetime
        ...
        jobs(ijob) = batch(cluster,@runSMOOTHERremote,1,{gfd_str,dirname,sitestr,ijob},'AdditionalPaths',clusterpaths,'AutoAddClientPath',false,'AutoAttachFiles',false);
