function vizufig=vizu_hpc(varargin)
% Plot GUISDAP results. This version does not open a figure window, but the result can be
% printed to file with saveas()
%--------------------------------------------------------------------------
...
WHICH_PARAM='Ne Te Ti Vi O+';  % Select parameters to plot (AE=Az,El,Power)
%%%%%%%%%% Y-Axis scales %%%%%%%%%%
...
if REALT
 ...
 if all(par1D(:,2)>75)
     SCALE(2,:)=[80 800];
     % a smaller altitude span for ESR ipy
     if strcmp(name_expr,'ipy')
         SCALE(2,:)=[80 500];
     end
 end
...
elseif ~REALT & ~strcmp(lower(act),'update')
 ...
 % this is the default, 24-hour plot around the median time
...
drawnow
if isempty(vizufig)
 vizufig=gupfigure_hpc;
 if ~local.x && round(10*local.matlabversion)==65
  close(vizufig),vizufig=gupfigure_hpc; % Matlab R13 bug
 end
...
function [varargout]=gupfigure_hpc(h)
if nargin==0
 h0=figure('visible','off');
elseif isempty(find(findobj('Type','Figure')==h))
 h0=figure(h,'visible','off');
else
