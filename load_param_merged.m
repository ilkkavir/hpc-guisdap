...
for k=1:length(fnames)
    ...
    if length(fname)==12
        if fname(1:4)=='data'
            list(l).file = str2num(fname(5:12));
            list(l).fname = fullfile(fileparts(data_path),[fname(5:12) '.mat']);
            l = l+1;
        end
    end
end
...
[r_param,r_error]=deal(zeros(0,max([max(myparams),5])));
...
%disp(list(i))
  fname = ['data' list(i).fname((end-11):(end-4))];
  tmpdata = getfield(load(data_path,fname),fname);
